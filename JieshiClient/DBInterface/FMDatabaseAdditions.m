...
- (void)setApplicationIDString:(NSString*)s {
    ...
    if ([s length] != 4) {
        debug_NSLog(@"setApplicationIDString: string passed is not exactly 4 chars long. (was %ld)", [s length]);
    }
    ...
    while (keepTrying == YES) {
        ...
        if (rc == SQLITE_BUSY || rc == SQLITE_LOCKED) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                debug_NSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                debug_NSLog(@"Database busy");
            }          
        } 
